...
export default class LiveSendMessage extends Component<{}, State, any> {
  _engine?: RtcEngine;
  ...
  onPressSend = async () => {
    const { message, listMessage } = this.state;
    if (!message) {
      return;
    }
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        {!isJoin && <TouchableOpacity
          onPress={this._initEngine}
          style={{
            paddingVertical: 10,
  ...
  _renderVideo = () => {
    const { remoteUid, listMessage } = this.state;
    return (
